...
dim_neigh AS (
    SELECT neighbourhood_dim_id, listing_neighbourhood 
...
metrics AS (
    ...
    FROM f
    JOIN dim_neigh dn ON dn.neighbourhood_dim_id = f.neighbourhood_dim_id
...
SELECT
    ...
    ROUND(100.0 * (active_listings - LAG(active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY month_year)) / NULLIF(LAG(active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY month_year), 0), 2) AS pct_change_active,
    
    ROUND(100.0 * ((total_listings - active_listings) - LAG(total_listings - active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY month_year)) / NULLIF(LAG(total_listings - active_listings) OVER (PARTITION BY listing_neighbourhood ORDER BY month_year), 0), 2) AS pct_change_inactive,
