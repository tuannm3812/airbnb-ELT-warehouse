...
cleaned AS (
    SELECT
        ...
        CASE
            ...
            WHEN
                scraped_date ~ '^\d{1,2}/\d{1,2}/\d{2,4}'
                THEN TO_DATE(scraped_date, 'DD/MM/YYYY')
        END AS scraped_date,
        ...
        CASE
            ...
            WHEN
                host_since ~ '^\d{1,2}/\d{1,2}/\d{2,4}'
                THEN TO_DATE(host_since, 'DD/MM/YYYY')
        END AS host_since,

        -- Boolean normalization
        COALESCE(LOWER(host_is_superhost) = 't', FALSE) AS host_is_superhost,
        ...
        COALESCE(LOWER(has_availability) = 't', FALSE) AS has_availability,
